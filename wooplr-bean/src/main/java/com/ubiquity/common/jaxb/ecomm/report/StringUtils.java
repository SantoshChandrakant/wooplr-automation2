...
import com.ubiquity.app.entity.fashionlooks.Constants;
...
public class StringUtils {
	...
	private static final String DOMAIN_PATTERN_STRING = "[a-z][a-z0-9\\-]{1,28}[a-z0-9]";
	private static final Pattern domainPattern = Pattern.compile(DOMAIN_PATTERN_STRING);

	public static final String preHtmlContent = "<html><head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">"
			+ "<meta name=\"HandheldFriendly\" content=\"True\">"

			+ "<link href=\"https://www.wooplr.com/styles/styles-1429104322902.css\" rel=\"stylesheet\" type=\"text/css\" />"
			+ "<link href=\"https://www.wooplr.com/styles/font-awesome.min.css\" rel=\"stylesheet\" type=\"text/css\" />"
			+ "<link href=\"https://www.wooplr.com/styles/facebox-1425893058611.css\" rel=\"stylesheet\" type=\"text/css\" />"
			+ "<script src=\"https://www.wooplr.com/js/guide-mobile.js\" type=\"text/javascript\"></script>" + "</head>"
			+ "<body class=\"guide-page mobile\">" + "<div id=\"board-container\" class=\"clearfix\">"
			+ "<article id=\"guide\">" + "<div class=\"content\"  style=\"margin-top:-10px\">";

	public static final String postHtmlContent = "</div></article></div></body></html>";
	...
	public static boolean isNullOrEmpty(ShortBlob str) {
		return str == null;
	}

	public static boolean isNullOrEmptyOrHash(String str) {
		return str == null || str.equals("") || str.equals("#");
	}

	public static Long StringToLong(String str) {
		return isNullOrEmpty(str) ? Constants.INVALID_LONG_ID : Long.valueOf(str);
	}
	...
	public static String getProfileHeading(String str, String entityType) {
		if (isNullOrEmpty(str)) // Just giving back what I got
			return str;
		String headingText = "Profile";
		if (entityType.equalsIgnoreCase(Constants.ATTRIBUTE)) {
			headingText = "Page";
		}
		return str.substring(str.length() - 1).equalsIgnoreCase("s") ? convertCase(str + "' " + headingText)
				: convertCase(str + "'s " + headingText);
	}

	public static String getStringAfterApostrophe(String str) {
		if (isNullOrEmpty(str)) // Just giving back what I got
			return str;
		return str.substring(str.length() - 1).equalsIgnoreCase("s") ? convertCase(str + "'")
				: convertCase(str + "'s ");
	}
	...
	public static String getShortEmailForSMS(String emailAddr, int maxLength) {
		...
		if (emailAddr.length() > maxLength) {
			...
			if (temp.length == 2) {
				...
				return emailAddr.substring(0, maxLength) + "...";
		} else
			return emailAddr;
	}
	...
	/*
	 * this function parses the string to escape emoji and special characters
	 * for HTML
	 */
	// public static String parseForHtml(String stringToParse) {
	// return
	// EmojiParser.parseToHtmlDecimal(StringUtils.escapeHTML(stringToParse));
	// }
	...
	public static void main(String[] args) {
		String[] tests = { "This is a test", "This@Th_at.com", "abcdef_", "_abcdef", "abc_def", "a!@#$%%^bCDEF",
				"abcd__ef", "1234567890" };
	 ...
	 *         (?=.*[a-z]) # must contains one lowercase characters (?=.*[A-Z])
	 *         # must contains one uppercase characters (?=.*[@#$%]) # must
	 *         contains one special symbols in the list "@#$%" . # match
	 *         anything with previous condition checking {6,20} # length at
	 *         least 6 characters and maximum of 20 ) # End of group
	 */
